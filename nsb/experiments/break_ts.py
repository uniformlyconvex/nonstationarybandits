...
class SwappingConstantArms(Experiment):
    ...
    def __init__(self) -> None:
        ...
        arms_mean = sum(SwappingConstantArms.ARM_VALUES) / len(
            SwappingConstantArms.ARM_VALUES
        )
        likelihoods = tuple(
            con.NormalLikelihood(
                loc_variance=con.NormalInverseGammaPrior(
                    mu=arms_mean, lambda_=1.0, alpha=2.0, beta=1.0
                )
            )
        ...
        super().__init__(self, get_agents())
...
class SineSwappingArms(Experiment):
    ...
    def __init__(self):
        ...
        likelihoods = tuple(
            con.NormalLikelihood(
                loc_variance=con.NormalInverseGammaPrior(
                    mu=self.ARM_MEAN, lambda_=1.0, alpha=2.0, beta=1.0
                )
            )
            for _ in range(2)
        )
